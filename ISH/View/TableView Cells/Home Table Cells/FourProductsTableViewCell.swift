//
//  FourProductsTableViewCell.swift
//  ISH
//
...
class FourProductsTableViewCell: UITableViewCell {
    ...
    @IBOutlet weak var itemFour: UIButton!{
        didSet{
            itemFour.setImage(nil, for: .normal)
        }
    }
    @IBOutlet weak var itemFourTxt: UILabel!
    @IBAction func itemFour(_ sender: UIButton) {
        goToProductDetailsPage(position: 3)
    }
    ...
    func printURL(){
            ...
            alamofireRequest(url: url, params: ["hashkey":AppDelegate.apiKey]) { (json, data) in
                do{
                    ...
                    let home = try JSONDecoder().decode(HologramRoot.self, from: data)
                    ...
                    self.itemFourTxt.text = self.prods[3].prodName
                    ...
                    alamofireImageRequest(url: self.prods[3].img, completion: { (img) in
                        self.itemFour.setImage(img.withRenderingMode(.alwaysOriginal), for: .normal)
                    })
